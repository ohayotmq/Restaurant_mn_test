...
class Student extends Component
{
    ...
    deleteStudent =async (e, id)=>{
        const deletetest =e.currentTarget;
        deletetest.innerText="Deleting";
        const res= await axios.delete(`http://localhost:8000/api/delete-student/${id}`);
        if(res.data.status===200)
        {   
            deletetest.closest("tr").remove();
                    ...
                    <div className="col-md-12">
                        <div className="card">
                            <div className="card-header">
                                <h4>Restaurants Data
                                    <Link to ="/add-student" className="btn btn-primary btn-sm float-end">Add Restaurant</Link>
                                </h4>
                            </div>
